...
const INTEGER_UNDERFLOW_ERR =
	"Result of current operation caused integer underflow";
const APP_NOT_FOUND =
	"RUNTIME_ERR1306: Application Index 9 not found or is invalid";
...
describe("DAO - Failing Paths", function () {
			...
			assert.throws(
				() =>
					ctx.executeTx({
						...withdrawVoteDepositTx,
						appArgs: [
							DAOActions.withdrawVoteDeposit,
							`int:${origDeposit + 5n}`,
						],
					}),
			...
			assert.throws(
				() =>
					ctx.executeTx([
						...
						{
							...clearVoteRecordTx,
							payFlags: { totalFee: 1000, note: "salt" },
						},
					]),
				RUNTIME_ERR1009
			);
	...
	describe("Close Proposal", function () {
		...
		let closeProposalTx;
		this.beforeEach(function () {
			closeProposalTx = mkCloseProposalTx(
				ctx.daoAppID,
				ctx.govTokenID,
				ctx.proposalLsig
			);
