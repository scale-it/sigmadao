...
class Context {
	...
	deployASA(name, creator) {
		this.govTokenID = this.runtime.deployASA(name, {
			creator: creator,
		}).assetIndex;
	}
	...
	setUpLsig() {
		...
		// compile lsig's
		this.daoFundLsig = this.runtime.loadLogic(
			"dao-fund-lsig.py",
			scInitParam,
			false
		);
	...
	addProposal() {
		...
		this.proposalLsigAcc.setLocalState(
			this.daoAppID,
			"id",
			parsing.stringToBytes("this-txID")
		);
	...
	depositVoteToken(voterAcc, amount) {
		const senderDeposit =
			voterAcc.getLocalState(this.daoAppID, "deposit") ?? 0n;
		voterAcc.setLocalState(
			this.daoAppID,
			"deposit",
			senderDeposit + BigInt(amount)
		);
		this.syncAccounts();
	}
	...
	vote(voterAcc, voteType, proposalLsigAcc) {
		...
		const abstainVotes =
			proposalLsigAcc.getLocalState(this.daoAppID, "abstain") ?? 0n;
		const senderDeposit =
			voterAcc.getLocalState(this.daoAppID, "deposit") ?? 0n;

		switch (voteType) {
			case Vote.YES:
				proposalLsigAcc.setLocalState(
					this.daoAppID,
					"yes",
					yesVotes + senderDeposit
				);
				break;
			case Vote.NO:
				proposalLsigAcc.setLocalState(
		...
		const depositLock =
			voterAcc.getLocalState(this.daoAppID, "deposit_lock") ?? 0n;
		const votingEnd = proposalLsigAcc.getLocalState(
			this.daoAppID,
			"voting_end"
		);
