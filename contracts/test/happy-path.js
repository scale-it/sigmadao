...
describe("DAO - Happy Paths", function () {
	...
	describe("Add proposal", function () {
		...
		it("should save proposal config in proposalLsig for ALGO transfer (type == 1)", function () {
			...
			assert.deepEqual(
				ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, "amount"),
				BigInt(2e6)
			);
		});
	...
	describe("Vote", function () {
		...
		it("should allow voterA to register deposited tokens as votes", function () {
			...
			assert.equal(
				ctx.voterA.getLocalState(ctx.daoAppID, "deposit_lock"),
				votingEnd
			);
		...
		it("should allow voterB to register gov tokens as votes after voterA", function () {
			...
			assert.equal(
				ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, "yes"),
				yesVotes + 6n
			);
			...
			assert.equal(
				ctx.voterB.getLocalState(ctx.daoAppID, "deposit_lock"),
				votingEnd
			);
			assert.equal(
				ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, "yes"),
		...
		it("should allow voting if already set proposal_id is different", function () {
			...
			ctx.voterA.setLocalState(
				ctx.daoAppID,
				key,
				parsing.stringToBytes("some-new-id")
			);
			ctx.syncAccounts();
	...
	describe("Execute", function () {
		...
		it("should execute proposal for type == 1 (ALGO TRANSFER)", function () {
			...
			assert.equal(
				ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, "executed"),
				1n
			);

			// verify payment recieved from dao fund
			assert.equal(
				ctx.proposer.balance(),
				beforeProposerBal + BigInt(2e6) - 2000n
			);
		});
		...
		it("should execute proposal for type == 2 (ASA TRANSFER)", function () {
			...
			const beforeProposerHolding = ctx.proposer.getAssetHolding(
				ctx.govTokenID
			).amount;
			executeProposalTx[1] = {
		...
		it("should execute proposal for type == 3 (MESSAGE)", function () {
			...
			const mp = ctx.proposalLsigAcc.appsLocalState.get(ctx.daoAppID)[
				"key-value"
			];
	...
	describe("Withdraw Vote Deposit", function () {
		...
		it("should allow withdrawal after voting is over", function () {
			...
			const beforeRecordVoterA = ctx.voterA.getLocalState(
				ctx.daoAppID,
				"deposit"
			);
			const beforeRecordVoterB = ctx.voterB.getLocalState(
				ctx.daoAppID,
				"deposit"
			);
			...
			assert.deepEqual(
				ctx.voterA.getAssetHolding(ctx.govTokenID).amount,
				beforeBalVoterA + 5n
			);
			assert.deepEqual(
				ctx.voterB.getAssetHolding(ctx.govTokenID).amount,
				beforeBalVoterB + 7n
			);
		});
	});
	...
	describe("Close Proposal", function () {
		...
		it("should close proposal if proposal is recorded & voting is not active", function () {
			...
			assert.equal(
				ctx.proposalLsigAcc.getAssetHolding(ctx.govTokenID).amount,
				BigInt(deposit)
			);
			...
			for (const key of [
				...
				assert.isUndefined(
					ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, key)
				);
			}
		});
	});
