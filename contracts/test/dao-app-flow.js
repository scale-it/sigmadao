...
describe("DAO test", function () {
	...
	it("DAO flow test", function () {
		...
		assert.deepEqual(
			proposalALsigAcc.getLocalState(appID, "voting_end"),
			BigInt(votingEnd)
		);
		assert.deepEqual(
		...
		assert.deepEqual(
			proposalALsigAcc.getLocalState(appID, "amount"),
			BigInt(2e6)
		);

		// verify deposit recieved in depositAcc
		assert.deepEqual(
			depositAcc.getAssetHolding(govTokenID).amount,
			beforeBal + 15n
		);
		...
		// verify sender.deposit is set
		assert.deepEqual(
			voterA.getLocalState(appID, "deposit"),
			beforeRecordVoterA - 6n
		);
		assert.deepEqual(
			voterB.getLocalState(appID, "deposit"),
			beforeRecordVoterB - 8n
		);

		// verify tokens received by voterA & voterB from depositAcc
		assert.deepEqual(
			voterA.getAssetHolding(govTokenID).amount,
			beforeBalVoterA + 6n
		);
		assert.deepEqual(
			voterB.getAssetHolding(govTokenID).amount,
			beforeBalVoterB + 8n
		);
	...
	it("Should allow to close proposal if it is passsed execution", function () {
		...
		assert.equal(
			proposerA.getAssetHolding(govTokenID).amount,
			proposerABalance + 1n
		);
		assert.equal(
			proposalALsigAcc.getAssetHolding(govTokenID).amount,
